...
Use this reference while analyzing `.md`, `.pdf`, `.pptx`, `.png`, `.jpg`, or `.jpeg` inputs before slide generation.
...
|--------|--------|
| `.md` | Skip to Phase 1 |
| `.pdf` | Use available document/PDF inspection tools to extract content → convert to slide markdown with `#` chapters and `---` separators |
| `.pptx` | Use available presentation inspection tools to extract slides → convert to slide markdown with `#` chapters and `---` separators |
| `.png`, `.jpg`, `.jpeg` | Use available image inspection tools to recognize content → convert to slide markdown |
...
5. **Image paths:** Do local image paths exist? Use `rg --files`, `find`, or the available file search tools to verify.
...
다음 단계: design reference를 사용해 디자인 강화 진행
